...
def compare_tags(lnum,line1,line2,tag,tagtype=None):
 if tagtype == None:
  regex = r'<%s>.*?</%s>' % (tag,tag)
 elif tagtype == 'n':
  regex = r'<%s n=".*?">.*?</%s>' % (tag,tag)
 else:
  print('compare_tags, unknown tagtype=',tagtype)
  exit(1)
 a1 = re.findall(regex,line1)
 a2 = re.findall(regex,line2)
 # return (a1,a2)
 if a1 != a2:
  n1 = len(a1)
  n2 = len(a2)
...
def compare(entries1,entries2):
 nd = 0
 ntag = 0
 # tags: ab, fr, ger, gk, lat, tib, toch
 # bot, zoo, lang, ed, hom, lex, ms, ls
 tagtype = None
 tag = 'ls'
 #tagtype='n'
 for ientry,e1 in enumerate(entries1):
  ...
  for iline,line1 in enumerate(e1.datalines):
   ...
   (a1,a2) = compare_tags(lnum,line1,line2,tag,tagtype=tagtype)
   if a1 != a2:
    nd = nd + 1
    print('ntag=',ntag)
    exit(1)
