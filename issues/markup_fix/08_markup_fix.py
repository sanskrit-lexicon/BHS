"""
Markup fixer + audit for bhs.txt (BHS).
Counterpart of PWG#175, PWK#113, and siblings.

BHS-specific notes: 169 non-standard <ab n> (Buddhist names). 1 <ab n='?'> placeholder.

Inputs: ../../../csl-orig/v02/bhs/bhs.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "bhs" / "bhs.txt", HERE / "bhs.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "bhs_fixed.txt"
...
TRIM_TAGS = ['ls', 'ab', 'lang', 'lex', 'tib', 'ed', 'fr', 'ger', 'hom', 'lat', 'ms', 'bot', 'toch', 'gk', 'zoo']
...
AUDIT_CHECKS = [
    ...
    ("Empty content tag", re.compile(r"<(ls|ab|lang|lex|tib|ed|fr|ger|hom|lat|ms|bot|toch|gk|zoo)\b[^>]*></\1>") if TRIM_TAGS else re.compile(r"(?!x)x")),
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for bhs.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"BHS markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against bhs.txt.\n")
        f.write(f"Paired tags: <ls>, <ab>, <lang>, <lex>, <tib>, <ed>, <fr>, <ger>, <hom>, <lat>, <ms>, <bot>, <toch>, <gk>, <zoo>\n")
        f.write(f"Notes: 169 non-standard <ab n> (Buddhist names). 1 <ab n='?'> placeholder.\n\n")
        f.write("AUTO-FIXES: nested <ab> + whitespace in <ls>, <ab>, <lang>, <lex>, <tib>, <ed>, <fr>, <ger>, <hom>, <lat>, <ms>, <bot>, <toch>, <gk>, <zoo>\n\n")
