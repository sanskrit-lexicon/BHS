...
frenchwords = spellcheckWords('fr')
...
spellchecker_wordlist('spellchecker_french.txt',frenchwords)

def isFrench(word):
 isfrench = (word in frenchwords)
 isenglish = (word in englishwords)
 return (isfrench and (not isenglish))
...
def find_french_words(entry,d):
 for line in entry.datalines:
  for m in re.finditer(r'{%(.*?)%}',line):
   ...
   words2 = [w for w in words1 if isFrench(w)]
...
def write(fileout,d):
 with codecs.open(fileout,"w","utf-8") as f:
  ...
  for key in keys:
   ...
   n = n + 1
   f.write(out+'\n')
 print(n,"French distinct words identified in",fileout)
